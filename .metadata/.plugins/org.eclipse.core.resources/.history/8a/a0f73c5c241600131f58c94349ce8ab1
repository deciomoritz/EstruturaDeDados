...
void listarTran() {
	if ((*listaAtual).vazia())
		...
		for (int i = 0; i <= (*listaAtual).verUltimo(); i++) {
			printf("(%i) %s %.2f\n", i, ((*listaAtual).mostrar(i).getNome()),
					(*listaAtual).mostrar(i).getValor());
		}
	}
}
...
void mostraSaldo() {
	...
	for (int i = 0; i <= listaCredito.verUltimo(); i++)
		tCredito += listaCredito.mostrar(i).getValor();
	for (int i = 0; i <= listaDebito.verUltimo(); i++)
		tDebito += listaDebito.mostrar(i).getValor();
