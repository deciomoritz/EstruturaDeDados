...
void listarTran() {
	if ((*listaAtual).vazia())
		...
		for (int i = 0; i <= (*listaAtual).getTamanho() - 1; i++) {
...
void remTran() {
	...
	while (!sair) {
		...
		switch (opcao) {
		case 't':
			while(!listaAtual->vazia())
				delete listaAtual->remover();
		...
		case 'r':
			while (true) {
				...
				if (myStream >> valor)
					if (valor >= 0 && valor <= listaAtual->getTamanho()-1)
						break;
				printf(
						"Número inválido... Entre com um"
								" número válido, inteiro, maior ou igual a 0 e menor ou igual a %i !\n",
						listaAtual->getTamanho()-1);
			}
			if (valor > listaAtual->getTamanho()-1)
